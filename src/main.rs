...
async fn main() {
    ...
    if let Ok(conn) = pool.get().await.unwrap() {
        db::create(&conn).expect("Failed to create db.");
    } else {
        panic!("Failed to connect to db");
    };
        ...
        MessageKind::Common(_) => {
            if let Some(text) = msg.text() {
                if let Some(user) = msg.from() {
                    ...
                    if let Ok(conn) = context.pool.get().await.unwrap() {
...
async fn perform_bulk_tasks(bot: AutoSend<Bot>, ctx: Arc<Context>) -> Result<bool, RequestError> {
    ...
    loop {
        ...
        {
            let messages = if let Ok(conn) = ctx.pool.get().await.unwrap() {
            ...
            for m in messages {
                ...
                for u in m.recipients {
                    ...
                    if let Ok(conn) = ctx.pool.get().await.unwrap() {
